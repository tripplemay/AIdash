...
| # | 任务 | 状态 | 备注 |
|---|------|------|------|
| 3.1 | Lesson 表新增 groupSize、aiRoundsCount 字段并迁移 | ⬜ 待启动 | 其他修复的前置依赖 |
| 3.2 | 详情页课次 pills 补充人数与 AI 回合显示 | ⬜ 待启动 | 依赖 3.1 |
| 3.3 | 列表页卡片底部补充人数与 AI 回合显示 | ⬜ 待启动 | 依赖 3.1 |
| 3.4 | Sidebar 移除无效菜单链接 | ⬜ 待启动 | 独立 |
| 3.5 | TopBar 移除无效导航链接 | ⬜ 待启动 | 独立 |
| 3.6 | 列表页搜索/筛选接入真实 API | ⬜ 待启动 | 独立 |
| 3.7 | 管理员后台增加 admin 角色校验 | ⬜ 待启动 | 独立 |
