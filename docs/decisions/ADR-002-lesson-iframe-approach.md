...
> ⚠️ 本方案已被 [ADR-003](ADR-003-content-spec-v2-lessonrenderer.md) 取代。当前系统采用 v2 结构化 JSON + LessonRenderer 方案，不再使用 iframe。本文档保留作为决策演进记录。
...
本方案在 2026-03-14 确认后短暂使用，随后被 ADR-003（v2 结构化 JSON + LessonRenderer）取代。取代原因见 [ADR-003](ADR-003-content-spec-v2-lessonrenderer.md)。
